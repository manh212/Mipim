...
const ApiSettingsScreen: React.FC<ApiSettingsScreenProps> = ({ setCurrentScreen, onSettingsSavedNotification }) => {
  ...
  const [useProxy, setUseProxy] = useState(DEFAULT_API_CONFIG.useProxy || false);
  const [proxyUrl, setProxyUrl] = useState(DEFAULT_API_CONFIG.proxyUrl || '');
  ...
  useEffect(() => {
    ...
    setUseProxy(loadedSettings.useProxy || false);
    setProxyUrl(loadedSettings.proxyUrl || DEFAULT_API_CONFIG.proxyUrl || '');
  }, []);
  ...
  const handleSaveSettings = () => {
    ...
    if (currentApiKeySource === 'user' && validKeys.length === 0 && !useProxy) {
      setError(VIETNAMESE.apiKeyRequiredError);
      return;
    }
     if (useProxy && !proxyUrl.trim()) {
      setError("URL Proxy không được để trống.");
      return;
    }
    setError('');
  ...
  const handleTestProxy = async () => {
    const urlToTest = proxyUrl.trim();
    if (!urlToTest) {
      setError("Vui lòng nhập URL proxy.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Proxy phản hồi với mã lỗi: ${response.status}`);
      }
      const data = await response.json();
      if (data.status === 'ok') {
        setSuccessMessage("Kết nối proxy thành công!");
      } else {
        throw new Error("Proxy phản hồi không hợp lệ.");
      }
    } catch (e) {
      const errorMsg = e instanceof Error ? e.message : 'Không rõ lỗi.';
      setError(`Kết nối proxy thất bại: ${errorMsg}`);
    } finally {
      ...
      <div className="w-full max-w-lg bg-gray-900 shadow-2xl rounded-xl p-6 sm:p-8">
        ...
        <div className="space-y-6">
          ...
          <div className="pt-4 border-t border-gray-700">
            <h3 className="text-lg font-semibold text-gray-200 mb-2">Cài Đặt Proxy (Tùy chọn)</h3>
             <div className="flex items-center justify-between p-3 bg-gray-700/50 rounded-md">
                <div>
                    <label htmlFor="use-proxy-toggle" className="font-medium text-gray-200 cursor-pointer">Sử dụng Proxy Server</label>
                    <p className="text-xs text-gray-400">Bật để gửi yêu cầu AI qua proxy cục bộ của bạn.</p>
                </div>
                <ToggleSwitch id="use-proxy-toggle" checked={useProxy} onChange={setUseProxy} />
            </div>
            {useProxy && (
                <div className="mt-3 pl-2 space-y-3">
                    ...
                    />
                    <Button onClick={handleTestProxy} isLoading={isTestingProxy} loadingText="Đang kiểm tra..." variant="secondary" size="sm" className="w-full">
                        Kiểm tra kết nối
                    </Button>
                </div>
            )}
       ...
       <p className="mt-8 text-xs text-gray-500 text-center max-w-md px-2">
        {useProxy ? "Game đang sử dụng Proxy Server. Cài đặt API Key sẽ không có tác dụng." : apiInfoText}
      </p>
    </div>
  );
};
